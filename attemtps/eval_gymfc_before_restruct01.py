...
class ModelEval:
	...
	def evalModel(self, toteps=1, err_thresh=.1):
		...
		for eps in range (toteps):
			...
			# Ep Stats
			stats = {'avg_r' : np.mean(ep_data['rewards']),  
				'fin_r' : r,
				'max_r' : max(ep_data['rewards']),
				'del_r' : abs(r-ep_data['rewards'][0]),
				'avg_rerr' : np.mean(ep_data['roll_err']),
				'avg_perr' : np.mean(ep_data['pitch_err']),
				'avg_yerr' : np.mean(ep_data['yaw_err']),
				'r_rise' : self.riseTime(ep_data['aroll_v'],ep_data['droll_v'][0],err_thresh),
				'p_rise' : self.riseTime(ep_data['apitch_v'],ep_data['dpitch_v'][0],err_thresh),
				'y_rise' : self.riseTime(ep_data['ayaw_v'],ep_data['dyaw_v'][0],err_thresh),
			}
			...
			self.eps_avg_rerr.append(stats['avg_rerr'])
			self.eps_avg_perr.append(stats['avg_perr'])
			self.eps_avg_yerr.append(stats['avg_yerr'])

			self.eps_r_rise.append(stats['r_rise'])
			self.eps_p_rise.append(stats['p_rise'])
			self.eps_y_rise.append(stats['y_rise'])
...
class ProcEval:
	def __init__(self, model_eval):
		if isinstance(model_eval, ModelEval): 
			self.me = model_eval
			self.eval_size = len(self.me.eps)
		else: raise Exception('{} is a {} not a ModelEval'.format(model_eval, type(model_eval)))
